...
当前实现覆盖 classic / evented_v2 / kernel_v3 三类编排模式，其中 `kernel_v3` 是默认生产执行核；`evented_v2` 是声明式 Loop Engineering 驱动的多 Agent 运行时基础设施，包含 durable run、AgentGraph、mailbox、run-local outbox、远程 agent worker/scheduler、worker registry、timeline/metrics 管理面，以及 shadow/canary/fallback 灰度闭环。基础运行面还包括 HTTP/SSE API、A2A task lifecycle、Skill/MCP/Web/Memory/Delegation provider、attachments/artifacts、hybrid SQLite+JSONL storage、Prometheus 指标、结构化 access log、OpenTelemetry HTTP tracing，以及工具输出预算、bash command audit、virtual path、terminal-state guard 等治理能力。

### evented_v2 与 kernel_v3 的关系

`kernel_v3` 和 `evented_v2` 不是简单的新旧版本替代关系，而是两个层级的编排能力：

- `kernel_v3` 是默认生产执行核，面向单 run/turn 的确定性执行，负责 checkpoint、effect idempotency、replay、lease/fence 与 provider-neutral tool handling。
- `evented_v2` 是声明式多 Agent 编排运行时，面向跨 Agent 的 graph 推进，负责 AgentGraph、handoff、mailbox、run-local outbox、remote worker/scheduler、worker registry、timeline/metrics 与 rollout 控制面。
- 生产路径上，`kernel_v3` 是默认 fallback 与稳定基线；`evented_v2` 通过 `runtime.eventedV2Rollout` 以 `shadow` / `canary` / `default` 逐步接管多 Agent 流量，并可通过 `fallbackMode: "kernel_v3"` 与 `autoFallback` 在失败率或连续失败触发时自动退回。

### 项目边界

KWorks 兼容层已从本仓库完全移除。Qiongqi 保持领域中立的通用 Agent 引擎，产品专属适配应在外部 adapter、部署配置或下游项目中完成，不进入核心引擎。
...
### evented_v2 生产 worker 与灰度

evented_v2 的生产 worker 可在不启动 HTTP server 的情况下运行 outbox reconciler 与 remote agent scheduler。生产编排系统可以先生成稳定 JSON 部署计划：
...
`runtime.eventedV2Rollout` 支持：

- `off`：保持 fallback mode，默认 `kernel_v3`。
- `shadow`：主路径仍走 fallback，同时记录 evented_v2 shadow intent 与决策指标。
- `canary`：按 `threadId` 稳定 hash 做 run 级流量分配。
- `default`：将 evented_v2 作为主运行时模式。
- `autoFallback`：按最近运行窗口的失败率、连续失败次数和冷却时间自动压回 `kernel_v3` / `classic`。
...
pnpm test                  # 全量测试（当前基线：124 个文件，883 个测试用例）
pnpm test:unit             # 单元测试
...
| `@qiongqi/loop` | kernel_v3 / evented_v2 / classic 编排，LoopRunner/LoopPlan，AgentGraph，mailbox/outbox，rollout |
...
### 🔧 Agent Loop
- **Kernel v3 默认生产核**：持久化 checkpoint、effect idempotency、provider-neutral tool handling，是当前默认生产执行模式
- **声明式 Loop Engineering**：evented_v2 模式演进为声明式 loop 基质——`LoopRunner` 解释 `LoopPlan` phase 集合（build-prompt → run-model → decide → 可选 evaluate → dispatch-tools），富事件（`prompt:built`/`model:ran`/`decision`/`tools:dispatched`/`step:retry`）真实化并写入 `LoopRun` 审计日志；可插拔确定性 `LoopEvaluator` 按 phase retry budget 触发有界 retry/reflection；classic 模式保留为回归锚
- **evented_v2 多 Agent runtime**：声明式 AgentGraph、durable run、mailbox、run-local outbox、远程 agent worker/scheduler、worker registry、timeline/metrics 管理面
- **生产灰度闭环**：`runtime.eventedV2Rollout` 支持 shadow/canary/default，按 run/thread 分流，并通过 `autoFallback` 自动回退到 fallback mode
...
│   ├── deployment.{zh,en}.md     # 生产部署、探针、指标、OTel、A2A、evented_v2 worker/rollout
│   ├── evented-v2-runtime.{zh,en}.md # evented_v2 与 kernel_v3 的关系、生产启用和剩余项
│   ├── packages/                 # 27 份逐包技术文档
...
├── tests/                         # 全量测试套件（当前基线：124 文件，883 测试）
...
| **阶段 4** | A2A 协议端点 | 本地闭环完成，外部跨厂商互操作待真实对端验证 |
| **Post-P1** | 运行治理 + OpenTelemetry exporter | 完成 |
| **Loop Engineering** | 声明式 loop 基质（LoopPlan/LoopRunner/Evaluator） | 完成 |
| **evented_v2 Runtime** | 声明式 AgentGraph + durable mailbox/outbox + remote worker/scheduler + timeline/metrics | 完成生产基础设施 |
| **生产部署与灰度** | worker pool / deployment-plan / shadow-canary-fallback rollout | 完成基础闭环 |
| **后续深化** | 隔离 shadow 双跑 diff、独立 graph manifest、能力约束与外部互操作 | 待后续控制面深化 |

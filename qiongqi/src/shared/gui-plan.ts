/**
 * Qiongqi-side mirror of the shared GUI plan contract from
 * DeepSeek-GUI's `src/shared/gui-plan.ts`.
 *
 * The renderer and the Qiongqi package live in the same repo but
 * TypeScript's `rootDir` constraint prevents Qiongqi from
...
export const GUI_PLAN_RELATIVE_DIR = '.qiongqisdd/plan'
...
export function validateCreatePlanToolInput(input: Partial<CreatePlanToolInput>): string[] {
  ...
  if (input.plan_relative_path != null) {
    ...
    if (!path) {
      ...
      issues.push('plan_relative_path must be a direct Markdown file under .qiongqisdd/plan')
    }
  }

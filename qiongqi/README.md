...
Qiongqi 是 KCoder 的本地 HTTP/SSE 代理运行时。它为 GUI 提供稳定、类型化且 GUI 友好的代理循环合约：

- `qiongqi serve` 会启动一个本地 HTTP 服务器，并暴露 `/v1/*` 路由。
...
```bash
qiongqi serve \
  --config ~/.kcoder/qiongqi/config.json \
  --host 127.0.0.1 \
  --port 8899 \
...
Qiongqi 也可以在无 GUI 的情况下独立运行：

```bash
qiongqi run --data-dir ~/.kcoder/qiongqi --workspace "$PWD" "summarize this repo"
qiongqi chat --data-dir ~/.kcoder/qiongqi --workspace "$PWD"
qiongqi exec --data-dir ~/.kcoder/qiongqi --workspace "$PWD" --list-tools
qiongqi exec --data-dir ~/.kcoder/qiongqi --workspace "$PWD" read --args '{"path":"README.md"}'
```

- `qiongqi run` 会创建一个线程，执行一个回合并流式输出助手文本后退出。
- `qiongqi chat` 启动行式 REPL。使用 `/exit`、`/quit` 或空行退出。
- `qiongqi exec --list-tools` 打印当前配置 / 工作区下生效的动态工具列表。
- `qiongqi exec <tool> --args <json>` 直接调用单个工具。`run` 或 `exec` 上可配合 `--json` 获取机器可读输出。

## 环境变量

当未通过 CLI 显式设置时，运行时会从 `process.env` 读取：

- `QIONGQI_CONFIG` – 明确的 JSON 配置文件路径
- `QIONGQI_HOST` – 监听主机（覆盖 `--host`）
- `QIONGQI_PORT` – 监听端口（覆盖 `--port`）
- `QIONGQI_DATA_DIR` – 数据目录（覆盖 `--data-dir`）
- `QIONGQI_RUNTIME_TOKEN` – 运行时 token（覆盖 `--runtime-token`）
- `QIONGQI_BASE_URL` – 模型 API 基础 URL（覆盖 `--base-url`）
- `DEEPSEEK_BASE_URL` – 备用模型 API URL
- `QIONGQI_MODEL` – 默认模型 ID（覆盖 `--model`）
- `DEEPSEEK_API_KEY` – 适配器转发到上游模型的 DeepSeek API key，GUI 默认运行时需要

## 配置文件

Qiongqi 使用 JSON 配置文件管理运行时行为，避免重建后重配或硬编码参数。
...
通过 `--config <path>` 或 `QIONGQI_CONFIG=<path>` 指定配置路径。
若未明确指定且设置了 `--data-dir` / `QIONGQI_DATA_DIR`，则会自动读取
`{data-dir}/config.json`（若存在）。GUI 默认路径是：

```text
~/.kcoder/qiongqi/config.json
```
...
Qiongqi 默认使用混合存储：`threads/{threadId}/messages.jsonl` 与 `events.jsonl` 是会话的标准回放日志；`index.sqlite3` 仅保存可重建的线程元数据（列表与搜索加速）。将 `serve.storage.backend` 设置为 `"file"` 可以回退到旧版 JSON 索引，或设置 `serve.storage.sqlitePath` 覆盖默认的 `{dataDir}/index.sqlite3`。

模型窗口、模型能力和模型级压缩阈值写在 `models.profiles`。内置配置已包含 `deepseek-v4-pro`、`deepseek-v4-flash` 以及兼容别名 `deepseek-chat` / `deepseek-reasoner`；DeepSeek V4 默认是 1M 上下文，并在约 980k input tokens 时开始压缩。旧的 `contextCompaction.modelProfiles` 仍会读取以兼容已有配置，但新配置请使用 `models.profiles`。更完整的文件位置、字段格式和用户自定义方式见 `../docs/QIONGQI_CONFIG.md`。
...
- `qiongqi run`/`qiongqi chat`/`qiongqi exec` 报错：核对 `--config`、`--data-dir`、`--api-key`、`--base-url`、`--runtime-token` 一致；先用 `qiongqi exec --list-tools --json` 检查工具注册表。
- 功能显示为 disabled：通常表示配置标志为 false；显示为 unavailable 通常表示标志为 true，但 provider/存储/模型未就绪或初始化失败。

## GUI 集成

Legacy provider 退役后，主流程由 `qiongqi-process.ts` 启动 Qiongqi，并通过 `runtimeRequest` 使用 bearer token 调用活动 base URL。渲染层保持与旧 `AgentProvider` 的接口兼容：`agents.qiongqi` 下保存 `binaryPath`、`port`、`autoStart`、`apiKey`、`baseUrl`、`runtimeToken`、`dataDir`、`model`、`approvalPolicy`、`sandboxMode`、`insecure`。
...
旧的持久化设置（`agentProvider: "codewhale"` 或 `"reasonix"`）会由 `migrateLegacyAppSettings` 做一次性迁移；旧凭据、baseUrl、端口与模型会映射到 `agents.qiongqi`；迁移后保存的设置不再包含 CodeWhale/Reasonix 配置。

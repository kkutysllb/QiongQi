...
- **`qiongqi worker` 是独立 remote worker 入口** —— CLI 可在不启动 HTTP server 的情况下创建 runtime 并驱动 `EventedV2OutboxReconciler` + `EventedV2RemoteAgentScheduler`；`--once` 适合批处理、探针和容器 job，daemon 模式等待 SIGINT/SIGTERM 后 shutdown；`--shard-index` / `--shard-count` 会在 runtime 创建前按稳定排序切分 `eventedV2AgentPeers`，让多个 worker 实例处理不同 agent 子集；`--plan --pool-size N` 只输出 shard 拓扑，不创建 runtime；`--pool-size auto` 会按 peer binding 数量自动规划 shard，最少 1 个 worker；非 `--plan` 的 `--pool-size N|auto` 会启动一个本地父 supervisor，并为每个 shard 拉起一个 child worker，父进程退出时统一 SIGTERM 子进程；child worker 非预期退出时会按 `--restart-backoff-ms`（默认 1000ms）重启同一 shard，超过 `--max-restarts` 后父 supervisor 以 runtime error 退出并终止整个 pool。
- **`qiongqi worker --deployment-plan` 是生产编排出口** —— CLI 会输出稳定 JSON：父 supervisor 命令、每个 shard child worker 命令、以及 `/health` / `/ready` / Prometheus / evented_v2 管理指标路径，供 Kubernetes、systemd、Nomad 或 CI 模板消费，不创建 runtime 也不启动 worker。
- **`runtime.eventedV2Rollout` 是灰度入口** —— 配置面支持 `off` / `shadow` / `canary` / `default`；`resolveEventedV2RolloutMode()` 用 `threadId` 稳定 hash 与 `canaryPercent` 做 run 级选择，保留 `fallbackMode`（默认 `kernel_v3`），`shadow` 阶段保持主路径不变并标记 `evented_v2` shadow intent；`EventedV2RolloutController` 记录 evented_v2 主路径结果，并按 `autoFallback.windowSize` / `minRuns` / `failureRateThreshold` / `consecutiveFailures` / `cooldownMs` 在冷却窗口内把后续 run 强制压回 fallback mode。
...
- `qiongqi worker --plan --pool-size auto sizes the worker pool from configured peer bindings`
...
- `qiongqi worker --max-restarts stops a crash-looping worker pool after a shard exceeds its restart budget`
...
- `RuntimeTuningConfigSchema accepts eventedV2Rollout.autoFallback and runtime factory uses shadow/canary/default policy to select the evented_v2 primary mode at run/thread granularity`
- `EventedV2RolloutController forces later evented_v2 canary/default runs back to fallback mode when recent failure-rate or consecutive-failure thresholds trip`

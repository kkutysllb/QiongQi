...
| `evented_v2` | `EventedTurnOrchestrator` + `LoopRunner`；额外挂载 `EventedV2MultiAgentRuntime`（durable run / mailbox / handoff / outbox flush shell）| 有（`LoopRun` / `TurnStateV2` + multi-agent `run.events` / `outbox` / `trace()`）| 多 Agent graph foundation、handoff、跨 Agent trace |
...
**第二阶段 run 级事务进展**：`EventedV2MultiAgentRuntime.handoff()` 会在 `MultiAgentRunStore.update()` 的同一 run 事务里写入 handoff 事件和 `mailbox_enqueue` outbox intent；run 提交后再投递 mailbox，成功后把对应 outbox 标记为 `published`。若进程在 run 提交后、mailbox 投递前/后崩溃，新的 runtime 实例可调用 `flushPendingOutbox(runId)` 重放单个 run 的 pending intent，也可通过 `MultiAgentRunStore.listWithPendingOutbox()` + `flushAllPendingOutbox()` 发现并批量恢复；`MailboxStore.enqueue` 对同一 message 保持幂等，且不会把 `delivered` / `completed` 状态降级回 `queued`。生产多 worker 落地仍需要外部调度器或 reconciler 周期性触发批量 flush。

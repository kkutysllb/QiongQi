...
>
> **版本说明**：本文档为 18 个 npm 包、阶段 1-3 已完成、阶段 4 基本完成（2026-06-22 基线）的真实状态。`README.*.md` 中的"16 个包"与阶段路线表已过时，以本文档为准。
>
...
| `/a2a/tasks` | POST 创建 / GET 列表（Stage 4）| Bearer |
...
| `/v1/runtime/info` / `/v1/runtime/tools` | 运行时诊断 | Bearer |
...
`createHttpServer({ agent, host, port })` 在 agent 上挂 HTTP 监听，返回 `QiongqiServeHandle`。
...
### 4.4 OrchestrationMode 双轨：classic vs evented

阶段 3 引入的事件化重构。`QiongqiServeRuntimeOptions.orchestrationMode: 'classic' | 'evented'`，默认 `classic`。

| 模式 | 编排器 | 崩溃恢复 | 适用场景 |
|------|-------|---------|---------|
| `classic` | `TurnOrchestrator`（显式 step 推进）| 无 | 默认，低开销 |
| `evented` | `EventedTurnOrchestrator`（订阅者协作）| 有（`FileTurnStateStore` 持久化 `TurnStateV1`）| 关键工作流、长回合 |

**共享逻辑**：`runOrchestratorStep` 纯函数被两个编排器共用——`EventedTurnOrchestrator` 把它包在 `TurnEventBus` 的 `step:start` / `step:end` 事件之间，订阅者可在 step 边界做协作。

**真实 backlog**：`createPromptSubscriber` 仍是占位，peer-style 编排是未来方向（设计哲学 §1.2 主张 ① 的诚实标注）。
...
- **A2A 任务端点**（阶段 4）— `POST /a2a/tasks` 创建并执行 turn；`GET /a2a/tasks/:id` 查询；`/cancel` / `/artifacts` / `/subscribe` SSE

**跨实例 A2A 闭环**（阶段 2 已验证）：Agent A 通过 `POST /a2a` 向 Agent B 提交任务，B 创建临时 thread、执行 turn、返回 `PeerArtifact`；A 通过 B 的 AgentCard 发现能力。
...
| **阶段 3** | TurnOrchestrator 事件化 | ✅ **完成** | TurnStateV1 / FileTurnStateStore / EventedTurnOrchestrator / TurnEventBus / runOrchestratorStep 共享函数 + kill -9 端到端崩溃恢复验证 |
| **阶段 4** | A2A 协议端点 | 🔄 **基本完成** | A2ATaskRecord / FileA2ATaskStore + `POST /a2a/tasks` + `GET /a2a/tasks/:id` + `cancel` + `artifacts` + SSE `subscribe` + ArtifactSchema 桥接。**待外部 Agent 做跨厂商互操作验证** |
...
```bash
pnpm test                  # 全量测试（53 个文件，433 个测试用例）

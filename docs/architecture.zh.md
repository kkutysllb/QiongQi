...
| `evented` | `EventedTurnOrchestrator` + `LoopRunner`（声明式 phase 解释）| 有（`FileTurnStateStore` 持久化 `LoopRun` / `TurnStateV2`，兼容升级 `TurnStateV1`）| 关键工作流、长回合 |

**共享策略**：classic 路径继续使用 `runOrchestratorStep`；evented 路径由 `LoopRunner` 解释 `LoopPlan.phases`（build-prompt → run-model → decide → evaluate → dispatch-tools），发布 `prompt:built` / `model:ran` / `decision` / `tools:dispatched` / `step:retry` 等 rich events，并把事件写入 `LoopRun.events`。`runStepViaEventBus` 仅作为兼容 API 保留。

...
| `evented_v2` | `EventedTurnOrchestrator` + `LoopRunner`；额外挂载 `EventedV2MultiAgentRuntime` + `EventedV2OutboxReconciler`（durable run / mailbox / handoff / outbox flush shell）| 有（`LoopRun` / `TurnStateV2` + multi-agent `run.events` / `outbox` / `trace()`）| 多 Agent graph foundation、handoff、跨 Agent trace |
...
**第二阶段 run 级事务进展**：`EventedV2MultiAgentRuntime.handoff()` 会在 `MultiAgentRunStore.update()` 的同一 run 事务里写入 handoff 事件和 `mailbox_enqueue` outbox intent；run 提交后再投递 mailbox，成功后把对应 outbox 标记为 `published`。若进程在 run 提交后、mailbox 投递前/后崩溃，新的 runtime 实例可调用 `flushPendingOutbox(runId)` 重放单个 run 的 pending intent，也可通过 `MultiAgentRunStore.listWithPendingOutbox()` + `flushAllPendingOutbox()` 发现并批量恢复；`EventedV2OutboxReconciler` 提供可周期运行、可停止、可观测的批量 flush 外壳；`MailboxStore.enqueue` 对同一 message 保持幂等，且不会把 `delivered` / `completed` 状态降级回 `queued`。生产多 worker 落地仍需要 server/worker lifecycle 决定是否自动启动 reconciler、调度间隔与错误上报策略。

**真实 backlog**：

- **P0：agent worker 生命周期** —— 已有 mailbox claim/complete 与 handoff mailbox，但还没有通用 agent worker loop 去 claim agent task、运行对应 agent、提交结果并推进 graph。
- **P0：graph interpreter** —— 当前 handoff 主要覆盖 manager-to-specialist happy path；还未完整解释声明式 Loop Engineering 图里的 wait / join / retry / judge / terminate / tool 节点。
- **P0：server/worker lifecycle 装配** —— `EventedV2OutboxReconciler` 已存在，但尚未由 HTTP/server runtime 自动启动；生产部署还需要 start/stop、健康检查、错误指标与配置项。
- **P1：跨实例 lease / CAS 深化** —— file store 已有 run-level lock 与幂等 outbox，但更强的多 worker 竞争控制、lease ownership、过期恢复与 store-native CAS/transaction 仍需下沉。
- **P1：观测与回放** —— 当前有 `trace()` 与 outbox flush result；还需要 run timeline、agent-run 状态投影、reconciler 指标、失败原因与管理 API。
- **P1：声明式配置面** —— 默认图仍是内置 manager-specialist；还需要从配置/manifest 加载通用 AgentGraph、agent binding、能力约束与 rollout 策略。
- **P2：分布式与远程 agent** —— A2A/Peer 基础已存在，但 evented_v2 的远程 agent 执行、artifact 回传、取消/超时/补偿策略还没有并入统一 graph runtime。
...
`/health` 适合作为 liveness；`/ready` 适合作为 readiness，并会在 hybrid SQLite fallback 时返回 `status=degraded`；Prometheus text endpoint 暴露 token/cache、A2A task 状态与存储降级状态。

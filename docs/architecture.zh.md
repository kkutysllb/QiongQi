...
>
> **版本说明**：本文档反映 2026-07-22 的生产运行基线：18 个 npm 包、四阶段架构本地闭环完成、`kernel_v3` 默认生产执行核、`evented_v2` 多 Agent runtime 与 shadow/canary/fallback 灰度闭环已落地。
> KWorks 兼容层已完全移除；Qiongqi 核心保持领域中立的通用 Agent 引擎。
>
...
`kernel_v3` 是默认生产执行核，负责单 run/turn 的确定性执行、checkpoint、effect commit、lease/fence、replay 与 provider-neutral tool handling。
`evented_v2` 是上层声明式多 Agent 编排运行时，负责 AgentGraph、handoff、mailbox、run-local outbox、remote worker/scheduler、worker registry、timeline/metrics 与 rollout 控制面。
`classic` 仅作为兼容 fallback，不扩展多 Agent 编排。

`evented` 是 legacy 兼容入口，会归一化到 `evented_v2`。显式选择 `evented_v2` 时会挂载 `EventedV2MultiAgentRuntime`；未显式选择时，生产默认仍是 `kernel_v3`，并通过 `runtime.eventedV2Rollout` 以 shadow/canary/default 的方式把多 Agent 流量逐步引入 evented_v2，必要时按 `fallbackMode` 自动退回 `kernel_v3` / `classic`。
...
**分层策略**：`kernel_v3` 提供默认生产执行核与 fallback 基线；`evented_v2` 提供多 Agent 编排 shell，并在需要时装配 evented turn loop、multi-agent run store、mailbox/outbox、remote scheduler 与 worker registry。二者通过 `runtime.eventedV2Rollout` 协同：`shadow` 不改变主路径但记录 evented_v2 shadow intent；`canary` 按 `threadId` 稳定 hash 做 run 级分流；`default` 让 evented_v2 成为主路径；`autoFallback` 会根据 evented_v2 主路径最近运行结果自动压回 `fallbackMode`。`classic` 路径继续使用 `runOrchestratorStep`；`LoopRunner` / `EventedTurnOrchestrator` 解释 `LoopPlan.phases`（build-prompt → run-model → decide → evaluate → dispatch-tools），发布 `prompt:built` / `model:ran` / `decision` / `tools:dispatched` / `step:retry` 等 rich events，并把事件写入 `LoopRun.events`。`runStepViaEventBus` 仅作为兼容 API 保留。
...
- **P2：分布式与远程 agent** —— `evented_v2` 远程执行基础已接入 mailbox + PeerRegistry + graph condition 推进，并已覆盖 artifact 回传、外部取消、超时转 aborted、mailbox claim lease/fence、server 内 remote scheduler lifecycle、scheduler supervisor metrics、store-backed worker heartbeat registry、声明式 peer outcome 补偿 condition 映射、mailbox completion outbox 恢复，以及不启动 HTTP server 的 `qiongqi worker --once` / daemon CLI 入口；`qiongqi worker --shard-index/--shard-count` 会在创建 runtime 前按稳定排序切分 `eventedV2AgentPeers`，让多个 worker 实例共享同一配置但处理不同 agent 子集；`qiongqi worker --plan --pool-size N` 可输出部署用 shard 拓扑而不创建 runtime，`--pool-size auto` 可按 peer binding 数量自动规划 shard；非 `--plan` 的 `qiongqi worker --pool-size N|auto` 会启动本地父 supervisor，并为每个 shard 拉起一个 child worker，父进程退出时统一终止子进程；child worker 非预期退出时会按 `--restart-backoff-ms`（默认 1000ms）重启同一 shard，超过 `--max-restarts` 后父 supervisor 以 runtime error 退出并终止整个 pool；`qiongqi worker --deployment-plan` 会输出父 supervisor、child worker 与探针/指标路径的稳定 JSON，供生产编排模板消费。
- **P2：生产灰度** —— `kernel_v3` 仍是默认生产执行核；`runtime.eventedV2Rollout` 已提供 `off` / `shadow` / `canary` / `default` 配置入口，`resolveEventedV2RolloutMode()` 会保留 `fallbackMode`（默认 `kernel_v3`），shadow 阶段不改变主路径并产生 evented_v2 shadow intent，canary 阶段按 `threadId` 稳定 hash 与 `canaryPercent` 在 run 级选择 `evented_v2`，default 阶段让 `evented_v2` 成为主模式。`EventedV2RolloutController` 已接入 canary/default 运行结果窗口，并按失败率、连续失败与冷却时间自动 fallback；后续默认承载多 Agent 流量前，还需继续深化隔离 shadow 双跑差异与灰度控制面的策略自动化。
...
| **阶段 4** | A2A 协议端点 | ✅ **本地闭环完成** | A2ATaskRecord / FileA2ATaskStore + 异步 `POST /a2a/tasks` + 同步兼容 `POST /a2a` + `GET /a2a/tasks/:id` + 可中断 `cancel` + `artifacts` + SSE `subscribe` + ArtifactSchema 桥接。外部跨厂商互操作仍需真实对端 |
| **evented_v2 Runtime** | 声明式多 Agent 编排 | ✅ **生产基础设施完成** | AgentGraph + durable run + mailbox/outbox + remote worker/scheduler + worker registry + timeline/metrics |
| **生产部署与灰度** | worker pool + rollout | ✅ **基础闭环完成** | `qiongqi worker --deployment-plan` + run/thread 级 shadow/canary/default + `autoFallback` |

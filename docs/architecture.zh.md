...
`createHttpServer({ agent, host, port, accessLog? })` 在 agent 上挂 HTTP 监听，返回 `QiongqiServeHandle`。`accessLog` 可接入 JSON logger / APM collector，收到的结构化条目只包含 request id、trace id、method、path、status、duration，不包含 authorization 等敏感 header。若请求带 W3C `traceparent`，运行时会透传响应头，并输出 `traceparent` / `traceId` / `spanId`。
...
**共享逻辑**：`runOrchestratorStep` 纯函数被两个编排器共用；evented 路径通过 `runStepViaEventBus` 发布 `step:start` / `step:end` 边界事件，订阅者可观察 step 边界并逐步接入协作逻辑。

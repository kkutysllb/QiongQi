...
`evented_v2` 是声明式多 Agent 编排层，负责 Agent graph、handoff、mailbox 与跨 Agent trace。
...
| `evented_v2` | `EventedTurnOrchestrator` + `LoopRunner`；额外挂载 `EventedV2MultiAgentRuntime`（durable run / mailbox / handoff shell）| 有（`LoopRun` / `TurnStateV2` + multi-agent `run.events` / `trace()`）| 多 Agent graph foundation、handoff、跨 Agent trace |

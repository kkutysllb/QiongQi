...
- **P2：分布式与远程 agent** —— `evented_v2` 远程执行基础已接入 mailbox + PeerRegistry + graph condition 推进，并已覆盖 artifact 回传、外部取消、超时转 aborted、mailbox claim lease/fence、server 内 remote scheduler lifecycle、声明式 peer outcome 补偿 condition 映射；后续还需独立 worker 进程管理、跨进程运行守护与补偿动作的事务化恢复。

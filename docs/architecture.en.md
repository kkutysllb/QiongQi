...
`/health` is suitable for liveness; `/ready` is suitable for readiness and returns `status=degraded` when hybrid SQLite falls back; the Prometheus text endpoint exports token/cache, A2A task status, evented_v2 run/agent/outbox state, remote scheduler supervision counters, and storage degraded state.

...
**Real backlog**: reconciler lease/flush metrics, standardized failure reasons, replay/rebuild queries with pagination/filtering, standalone manifest file loading, agent binding execution policy, capability constraints, graph-level rollout config, worker process-pool supervision beyond the new `qiongqi worker` CLI entrypoint, and production shadow/canary rollout remain next. `createPromptSubscriber` is still a placeholder; peer-style orchestration is a future direction (the honest annotation on Proposition ① in §1.2).

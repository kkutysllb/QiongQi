...
| `evented_v2` | `EventedTurnOrchestrator` + `LoopRunner`, with `EventedV2MultiAgentRuntime` + `EventedV2OutboxReconciler` + `EventedV2RemoteAgentScheduler` for durable run / mailbox / handoff / outbox flush / remote worker polling | yes (`LoopRun` / `TurnStateV2` plus multi-agent `run.events` / `outbox` / `trace()` / `timeline()` / `metrics()`) | multi-Agent graph foundation, handoff, cross-Agent trace |
...
**P1-B observability progress**: `@qiongqi/loop` now exposes `buildEventedV2RunTimeline(run)` and `buildEventedV2RunMetrics(runs)` projections; `EventedV2MultiAgentRuntime.timeline(runId)` / `metrics()` provide read-only runtime APIs; and `MultiAgentRunStore.listAll()` is the store-native full-run enumeration capability. HTTP management adds `GET /v1/runtime/evented-v2/runs/:runId/timeline` and `GET /v1/runtime/evented-v2/metrics`, returning `capability_unavailable` when no `evented_v2` runtime is configured. Prometheus `/v1/runtime/metrics?format=prometheus` appends `qiongqi_evented_v2_runs_total{status=...}`, `qiongqi_evented_v2_outbox_pending`, and `qiongqi_evented_v2_agent_runs_total{status=...}` when the multi-agent runtime is mounted; when the remote scheduler / worker registry is mounted, it also appends `qiongqi_evented_v2_remote_scheduler_*` and `qiongqi_evented_v2_workers_*` running, flush, message, error, online, and expired counters.

**P1-C declarative config progress**: `RuntimeTuningConfigSchema` now accepts `runtime.eventedV2AgentGraph`, reusing `AgentGraphSchema`. When `orchestrationMode=evented_v2`, `createAgent()` prefers that declarative graph and validates it with `validateAgentGraph()`, rejecting unknown nodes, duplicate edge conditions, and other invalid topology. When absent, the runtime keeps the default manager-specialist graph. This lets evented_v2 load generic `agent` / `handoff` / `wait` / `tool` / `judge` / `join` / `retry` / `terminate` topologies from config.

**P2-A remote Agent execution progress**: `RuntimeTuningConfigSchema` now accepts `runtime.eventedV2AgentPeers`, meaning `agentId -> AgentCard.id`; `runtime.eventedV2RemoteAgent.timeoutMs` configures remote invocation timeout, `leaseTtlMs` enables mailbox claim leases, `heartbeatTtlMs` configures the worker registry heartbeat expiry window, `scheduler.enabled` / `scheduler.intervalMs` starts periodic remote agent polling, and `compensation.statusConditions` maps peer outcomes into declarative graph conditions. When `orchestrationMode=evented_v2` and peer bindings are configured, the server runtime mounts `EventedV2RemoteAgentWorker`. The worker claims an agent task from the generic mailbox, converts it into a `PeerTask`, invokes the shared `PeerRegistry.invokePeer()` local/remote peer path, then maps `PeerArtifact.status` into a graph condition (default `completed` / `failed` / `aborted`, optionally configured as compensation edges such as `remote_failed`) and submits it through `EventedV2MultiAgentRuntime.completeAgentTask()`. Agent completion records mailbox terminal completion as a run-local `mailbox_complete` outbox intent, so remote failed / aborted / compensation conditions can recover mailbox terminal status transactionally through the reconciler after run advancement. `EventedV2RemoteAgentScheduler` polls the configured peer-bound agents, isolates per-agent errors, reports processed messages, and stops with server runtime shutdown; `snapshot()` exposes workerId, running/stopped status, health, flush/message/error counters, and heartbeat timestamps, and each flush records a remote agent worker heartbeat in the generic `EventedV2WorkerRegistryStore`. The in-memory and file stores both implement that registry, and HTTP runtime metrics plus Prometheus project worker total/online/expired counts by role. `PeerArtifact.artifacts` is persisted as `agentRuns[].peerArtifact` and projected into the run timeline; remote failed / aborted outcomes update both the agent run and mailbox terminal status. Mailbox claim leases use holder / TTL / fence metadata to prevent multiple workers from completing the same message while allowing expired delivered messages to be reclaimed. This reuses the existing A2A/PeerRegistry layer without putting HTTP transport or product-specific protocol into `@qiongqi/loop`.

**Real backlog**: standalone manifest file loading, agent binding execution policy, capability constraints, isolated shadow dual-run diffing, and deeper evented_v2 control-plane automation remain next. The current production surface now includes the local `qiongqi worker --pool-size N|auto` supervisor with restart backoff and max restart budgets, `qiongqi worker --deployment-plan` for stable deployment commands/probes, and `runtime.eventedV2Rollout` for run/thread-level `off` / `shadow` / `canary` / `default` selection with fallback mode preservation plus automatic fallback from recent failure-rate / consecutive-failure windows. `createPromptSubscriber` is still a placeholder; peer-style orchestration is a future direction (the honest annotation on Proposition ① in §1.2).
...
`/health` is suitable for liveness; `/ready` is suitable for readiness and returns `status=degraded` when hybrid SQLite falls back; the Prometheus text endpoint exports token/cache, A2A task status, evented_v2 run/agent/outbox state, remote scheduler supervision counters, worker registry online/expired counters, and storage degraded state.

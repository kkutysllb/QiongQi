...
export const DEFAULT_WORK_MODES = {
  ...
  finance: {
    ...
    description: [
      ...
      '',
      '## 你的技能工具箱（必须强制使用）',
      '你具备该工作模式下的所有技能，任何分析都必须主动调用它们。金融数据源优先级是硬性执行顺序：先调用当前技能绑定的官方 Tushare/iWencai 接口；只有技能明确返回凭证缺失、权限不足、接口失败、超时或空数据时，才允许 web_search/web_fetch，并在 Web 工具参数中填写 primary_source_attempted=true 与 fallback_reason。禁止在技能调用前主动 Web 搜索。最终回答必须标注实际数据源、数据日期和降级原因；Web 结果不能替代已经成功取得的官方数据。凭证由运行时按当前用户注入，禁止写入普通配置或提示词。分析类任务的结果必须严格按照分析报告技能的要求维度完成。',
      '',

...
const TOOL_DESCRIPTION = [
  ...
  '.qiongqisdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .qiongqisdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.qiongqisdd/plan/foo.md`)
 ...
 *    `.qiongqisdd/plan/<feature>.md` under the active workspace.
...
function resolveReservedTarget(
  ...
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .qiongqisdd/plan' }
  }
  if (input.operation === 'draft' && !isGuiPlanCurrentRelativePath(relativePath)) {
    return { error: 'legacy .qiongqi/plan paths can only be refined' }
  }
 ...
 * allocates a fresh, non-colliding `.qiongqisdd/plan/<feature>.md`.
 */

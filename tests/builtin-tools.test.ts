...
import { createReadTool as createReadToolFromModule } from '@qiongqi/adapter-tools'
import { createBashTool as createBashToolFromModule } from '@qiongqi/adapter-tools'
import { createEditTool as createEditToolFromModule } from '@qiongqi/adapter-tools'
import { createFindTool as createFindToolFromModule } from '@qiongqi/adapter-tools'
import { createGrepTool as createGrepToolFromModule } from '@qiongqi/adapter-tools'
import { createLsTool as createLsToolFromModule } from '@qiongqi/adapter-tools'
import { createWriteTool as createWriteToolFromModule } from '@qiongqi/adapter-tools'
import { computeEditDiff } from '@qiongqi/adapter-tools'
import { withFileMutationQueue } from '@qiongqi/adapter-tools'
import { DEFAULT_MAX_BYTES } from '@qiongqi/adapter-tools'
import type { TurnItem } from '@qiongqi/contracts'
import type { FsStats } from '@qiongqi/adapter-tools'
import type { ToolHostContext, UserInputRequest } from '@qiongqi/ports'
...
describe('Qiongqi built-in tools', () => {
  ...
  it('advertises structured GUI input choices and normalizes single-question options', async () => {
    ...
    let seenInput: Omit<UserInputRequest, 'threadId' | 'turnId'> | null = null
    const result = await host.execute(
      {
        callId: 'call_input',

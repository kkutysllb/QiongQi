...
import type { ServeOptions } from '@qiongqi/cli'
...
import { GOAL_TOOL_NAMES } from '@qiongqi/adapter-tools'
...
describe('Qiongqi agent CLI commands', () => {
  let dataDir = ''
  ...
  it('splits explicit commands and keeps legacy serve flags compatible', () => {
    expect(splitQiongqiCliCommand(['run', 'hello'])).toEqual({ command: 'run', args: ['hello'] })
    expect(splitQiongqiCliCommand(['--port', '9999'])).toEqual({
      command: 'serve',
      args: ['--port', '9999']
    })
    expect(splitQiongqiCliCommand(['nope']).error).toMatch(/unknown command/)
  })

...
import { createThreadRecord, touchThread } from '@qiongqi/domain'
import { createTurnRecord, startTurn } from '@qiongqi/domain'
import { makeAssistantTextItem, makeToolCallItem, makeToolResultItem, makeUserItem } from '@qiongqi/domain'
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    ...
    try {
      const relativePath = '.qiongqisdd/plan/demo.md'
...
describe('ThreadService.resumeSession', () => {
  it('uses the Qiongqi default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
    ...
    expect(result.thread.model).toBe(DEFAULT_QIONGQI_MODEL)
  })
})
